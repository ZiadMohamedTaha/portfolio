...
import pickle
import util
from test import test  # Assuming test function is defined in test.py
...
class App:
    ...
    def load_logo(self):
        path_logo = "C:\\Users\\USER\\Pictures\\Camera Roll\\Screenshot 2024-06-11 145959.jpg"  # Replace with your logo path
        image = Image.open(path_logo)
        image = image.resize((300, 300), Image.ANTIALIAS)  # Resize the image to fit in the window
    ...
    def take_id_card(self):
        ...
        file_path = os.path.join(self.db_dir, '{}__foullward_id_card.pickle'.format(self.user_save))
        with open(file_path, 'wb') as file:
            pickle.dump(self.id_card_img, file)
    ...
    def back_id_card(self):
        ...
        file_path = os.path.join(self.db_dir, '{}__back_id_card.pickle'.format(self.user_save))
        with open(file_path, 'wb') as file:
            pickle.dump(self.back_picture, file)
    ...
    def save_and_operate(self):
        ...
        for face_encoding in face_encodings:
            ...
            label = test(
                image=webcam_image,
                model_dir=r"C:\Users\USER\Downloads\Silent-Face-Anti-Spoofing-master\Silent-Face-Anti-Spoofing-master\resources\anti_spoof_models",
                device_id=0
            )
